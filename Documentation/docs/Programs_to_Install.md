...
pip3 list
```
If you get back a list of python packages installed on your computer, you have ``pip3`` installed. E.g.
...
If you do not see this, you probably do not have ``pip3`` installed on your computer. If this is the case, check out this [PIP Installation guide](https://pip.pypa.io/en/stable/installation). 

!!! note

    You may be used to this being called ``pip`` rather than ``pip3``. In most cases, ``pip`` and ``pip3`` are synonymous for the Python Installation Package for Python 3. **However in some cases,** ``pip`` **will be directed to the Python Installation Package for Python 2 rather than Python 3.** For clarity, we will stick to using ``pip3`` to refer to pip.


## Packaging

The ``packaging`` program is also used in this program to check the versions of ASE that you are using for compatibility issues. The easiest way to install ``packaging`` is through ``pip3``. Type the following into the terminal:
...
The ``numpy`` program is used in this program to perform matrix calculations, as well as used by ``ase``. The easiest way to install ``numpy`` is through ``pip3``. Type the following into the terminal:
...
The ``scipy`` program is used by the ``ase`` program. The easiest way to install ``scipy`` is through ``pip3``. Type the following into the terminal:
...
The ``matplotlib`` program is used in this program to create the energy profile plots made by the ``viewORCA neb_snap`` module. The easiest way to install ``matplotlib`` is through ``pip3``. Type the following into the terminal:
...
The installation of ASE can be found on the [ASE installation page](https://ase-lib.org/install.html), however from experience if you are using ASE for the first time, it is best to install ASE using ``pip3``, the package manager that is an extension of python to keep all your programs easily managed and easy to import into your python. 

To install ASE using ``pip3``, perform the following in your terminal.
...
Installing using ``pip3`` ensures that ASE is being installed to be used by Python 3, and not Python 2. Installing ASE like this will also install all the requisite programs needed for ASE. This installation includes the use of features such as viewing the xyz files of structures and looking at ase databases through a website. These should already be accessible, which you can test by entering into the terminal:
...
<figure markdown="span">
  ...
  <figcaption>This is a blank ase gui screen that you would see if you enter ase gui into the terminal.</figcaption>
</figure>
...
```bash
user@computer_name docs % pip3 show ase
Name: ase
Version: 3.26.0
...
The [``viewORCA`` program](https://geoffreyweal.github.io/viewORCA) is used throughout this procedure to allow the user to view various ORCA calculations. The easiest way to install ``viewORCA`` is through ``pip3``. Type the following into the terminal:
