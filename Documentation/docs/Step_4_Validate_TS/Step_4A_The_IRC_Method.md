...
An example of the complete ``orca.inp`` file for running a IRC calculation in ORCA is given below (also [located here](https://github.com/geoffreyweal/ORCA_Mechanism_Procedure/blob/main/Examples/Step4A_Validate_TS/orca.inp)): 
...
Look at the end of the [``output.out`` file](https://github.com/geoffreyweal/ORCA_Mechanism_Procedure/blob/main/Examples/Step4A_Validate_TS/output.out#L2877) and makes sure there is some sort of message that says that ORCA finished successfully, like that below:
...
See an [example of this below](https://github.com/geoffreyweal/ORCA_Mechanism_Procedure/blob/main/Examples/Step4A_Validate_TS/output.out#L2675):
...
Below show an [example of this from ``Examples/Step4A_Validate_TS/output.log``](https://github.com/geoffreyweal/ORCA_Mechanism_Procedure/blob/main/Examples/Step4A_Validate_TS/output.out#L2781). You can see that the transition step occurs at step 37, as expected. We also see that the energy of the transition state is the largest for all steps. 

```title="Example of IRC SUMMARY text from the ORCA output file"
---------------------------------------------------------------
                       IRC PATH SUMMARY              
---------------------------------------------------------------
...
We can also repeat **Check 2** by looking at the energy profile from the [``orca_IRC_Full_trj.xyz`` file](https://github.com/geoffreyweal/ORCA_Mechanism_Procedure/blob/main/Examples/Step4A_Validate_TS/orca_IRC_Full_trj.xyz) so that we can see how the IRC ran visually. This is a good idea as an extra check. 
