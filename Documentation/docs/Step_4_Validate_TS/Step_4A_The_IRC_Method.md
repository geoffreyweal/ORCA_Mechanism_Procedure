...
!!! note "Read the theory first"
    ...
    Before starting this step, we recommend reading the [theory for this step](../Theory.md#step-4-validate-the-transition-state) to understand why we are performing this step.
...
!!! note "Notes about the ``TolMAXG`` and ``TolRMSG`` keywords"
    ...
    2. It is recommended to set ``TolMAXG`` and ``TolRMSG`` to values equivalent to the ``LooseOPT`` keyword. This is because IRC can take a very long time to run if you use tighter convergence criteria. We will check the outputs from the IRC method with the ``TightOPT`` keyword in Step 4B. 
    3. While we are using the default settings for ``TolMAXG`` and ``TolRMSG`` (set to that of ``LooseOPT``), you can change this. If you want to tighten the geometric convergence criteria for the IRC method, see the settings below. However, for this procedure it is recommended to keep the ``LooseOPT`` settings for ``TolMAXG`` and ``TolRMSG``, and use tighter convergence settings in [Step 4B](Step_4B_Local_Optimisation.md). 
...
An example of the complete ``orca.inp`` file for running an IRC calculation in ORCA is given below (also [located here](https://github.com/geoffreyweal/ORCA_Mechanism_Procedure/blob/main/Examples/Step4_Validate_TS/Step4A_IRC/orca.inp)): 

```title="Example of an ORCA input file for an IRC calculation"
...
ORCA will create a number of files. These are the important ones to look at for the IRC method:

* ``output.out``: We will want to look at this file to make sure the IRC method has not run into any problems. 
* ``orca_IRC_F_trj.xyz``: These are the geometric optimisation steps performed for the forward step.
* ``orca_IRC_B_trj.xyz``: These are the geometric optimisation steps performed for the backwards step.
* ``orca_IRC_Full_trj.xyz``: These are the geometric optimisation steps performed for the full IRC method. These include the steps from the ``orca_IRC_B_trj.xyz`` and ``orca_IRC_F_trj.xyz`` files. 

!!! note

    You will find that the output from the ``Forward`` step may be either the reactant or the product. So don't worry if the ``Forward`` step gives you your reactant. This only means that in that case your ``Backward`` step should be your product. 
...
### Check 2: Make sure that the energy of each geometric optimisation step is less than that of the transition state from the ``output.out`` file

We want to make sure that the energy of our transition state is in fact higher than all points along the energy profile. This is because:

* The IRC method works by initially pushing the molecule down the saddle of the potential energy surface towards the reactant and product minima, so the energy should have decreased from the transition state in the forward and backwards directions. 
...
Shown below is an [example of this from ``Examples/Step4_Validate_TS/Step4A_IRC/output.out``](https://github.com/geoffreyweal/ORCA_Mechanism_Procedure/blob/main/Examples/Step4_Validate_TS/Step4A_IRC/output.out#L2103). You can see that the transition state occurs at step 74, as expected. We also see that the energy of the transition state is the largest for all steps. 
...
### Check 3 (Optional): Make sure that the energy of each geometric optimisation step is less than that of the transition state using the ``viewORCA irc`` program
...
!!! note
    ...
    * If you just want to create the ``IRC_images.xyz`` file, type into the terminal ``viewORCA irc --view False`` (which will create the ``IRC_images.xyz`` file without creating an ``ase gui`` window). 
...
If you put the cursor over the energy profile, you will see in this example that the maximum peak occurs at ``x=73``. Because ASE starts at ``0`` rather than ``1``, ``x=73`` here means ``step 74``, so we are happy. 
