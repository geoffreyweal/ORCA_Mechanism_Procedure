...
!!! note "Read the theory first"
    ...
    Before starting this step, we recommend reading the [theory for this step](Theory.md#step-1-locally-optimise-the-reactants-and-products) to understand why we are performing this step.
...
* ``FREQ``: Indicates you want ORCA to calculate the vibrational frequency for your molecule. 
...
!!! note "**NOTE 2**"
    ...
    [Click here](https://sites.google.com/site/orcainputlibrary/numerical-precision?authuser=0) for more information about other electronic convergence and integration grid settings (besides ``TightSCF`` and ``defgrid2``)
...
3. The overall multiplicity of your system.
4. If you use the ``xyzfile`` type, the xyz file of your system. 

Here, ``xyzfile`` allows you to import an xyz file into ORCA. You can add the xyz data directly in the ``.inp`` file, but I find having a separate ``xyz`` file is better because this allows you to look at the xyz file in a GUI like in [atomic simulation environment (ASE)](https://ase-lib.org/ase/gui/basics.html). 
...
???+ tip "**What is the multiplicity of my system?**"
    ...
    * **Example 3**: Octahedral Fe(III) complexes have 5 valence electrons. <u>In the high spin case</u>, 3 of these electrons live individually in each of the 3 t<sub>2g</sub>, while the other 2 electrons live in the 2 e<sub>g</sub> orbitals. Therefore, the total electron spin is $5/2$, giving a multiplicity of $M = 2(5/2) + 1 = 6$.

    <figure markdown="span">
        <img src="Figures/Electron_Multiplicity/Multiplicity_of_metals.png?raw=true" alt="Multiplicity_of_metals.png" width="800"/>
        <figcaption>3d orbitals of octahedral Cu(I) (left), Cu(II) (middle) and Fe(III) (right) based on crystal field theory.</figcaption>
    </figure>
...
!!! note "**NOTE 4**"
    ...
    ORCA has been told to use ``maxcore`` = 2000 (MB) per core in the ``.inp`` file, and slurm has been asked to reserve the same amount of memory per CPU (``--mem-per-cpu=2000MB``), giving this job 2000 MB * 32 = 64 GB of memory in total.

    Be aware that ``maxcore`` is a guide rather than a hard limit: ORCA will sometimes use a bit more memory per core than you have given it. If this happens, slurm will cancel your job for exceeding its memory allocation. If you find your jobs being cancelled for this reason, increase ``--mem-per-cpu`` in your ``submit.sl`` file by a few hundred MB (for example, from ``2000MB`` to ``2500MB``) while leaving ``maxcore`` at 2000 in your ``.inp`` file. This gives ORCA a bit of headroom to go over its allocation without changing how much memory ORCA thinks it is allowed to use.
...
You want to look in the [``output.out`` file](https://github.com/geoffreyweal/ORCA_Mechanism_Procedure/blob/main/Examples/Step1_Geo_Opt/Products/output.out#L19290) for a table with the title ``Geometry convergence``. There will be many of these tables, as one is given for each geometric step performed. 
...
After performing a local optimisation, it is important that you look at the vibrational frequencies that are calculated. These are the frequencies that you could see in an IR or Raman spectrum. You want to look through your [ORCA ``output.out`` file](https://github.com/geoffreyweal/ORCA_Mechanism_Procedure/blob/main/Examples/Step1_Geo_Opt/Products/output.out#L20882) for a heading called ``VIBRATIONAL FREQUENCIES``. **We want to make sure that all the frequencies are non-negative**. This means we are in a local energy well ([see the Theory page for why this works](Theory.md#vibrational-frequency-analysis-can-help-determine-if-we-are-in-a-local-minimum)). 
...
In the [example below (for ``Products``)](https://github.com/geoffreyweal/ORCA_Mechanism_Procedure/blob/main/Examples/Step1_Geo_Opt/Products/output.out#L20882), you can see that there are no negative frequencies from the ``FREQ`` calculation, therefore we are in a local energy well: 
...
!!! note
    ...
    Also read more about electronic convergence issues that are commonly encountered during geometric optimisation jobs from the ORCA Input Library by [clicking here](https://sites.google.com/site/orcainputlibrary/scf-convergence-issues).
