...
We now can obtain the energies and structures for the reactant, product, and transition state. To do this, we want to compare the energies of the ``Reactant`` and ``Product`` with the ``Forwards`` and ``Backwards`` structures from Step 4B. It is good to do this because sometimes the structures from 4B might be slightly lower energy than what was obtained from Step 1. 

!!! note "**NOTE 1**"

	We want to take the Gibbs Free Energy, so make sure you take this energy value from the ``output.out`` files. We can obtain this value because we have run the vibrational frequency calculation, which allows the vibrational entropy to be calculated. You will find these if you look for the ``GIBBS FREE ENERGY`` header in the ``output.out`` file. 

!!! note "**NOTE 2**"

	The energy values you get will be in hartrees (units are Eh or Ha). It is important to record the whole hartrees energy value to the full decimal places. 
...
!!! note "**NOTE 4**"
	...
	The conversion from Hartrees (Eh or Ha) to kJ/mol is to multiply the Hartree energy by 2625.5 to get the energy in kJ/mol.
...
* The energy from the reactant to transition state is $-1967.01477769 \rm{Eh} - -1967.08247260 \rm{Eh} = 0.06769491 \rm{Eh}$ which in kJ/mol is $0.06769491 \rm{Eh} \times 2625.5 = 177.7 \rm{kJ/mol}$ (1dp).
...
From this result, the activation energy for this reaction is 178 kJ/mol. 
