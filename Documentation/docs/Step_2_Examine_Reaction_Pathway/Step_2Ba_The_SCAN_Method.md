...
1. Perform a geometric optimisation on the chemical system, where the distance between two atoms is fixed.
2. Repeat the geometric optimisation on the same chemical system, where the distance between the two atoms is changed by $dx$.
...
* ``NormalOPT``: Indicates we want to use normal geometric convergence settings. 
...
!!! note "**NOTE 2**"
    ...
    The energy goes up at the end because the hydrogen atom is probably getting too close to the carbon atom. We can ignore this part of the SCAN (after around step 110), as it is not relevant to us for finding the transition state for this mechanistic step. 

The easiest way to use this GUI is to zoom in on the part of the energy profile that looks like the transition state (by clicking on the <img src="../Figures/2Ba_SCAN/Magnifying_Glass.png?raw=true" alt="drawing" width="25"/> button), and from this determine the image this corresponds to by looking at the x axis. Move the cursor over the highest point along the energy profile, and read the x value:
...
<figure markdown="span">
    <img src="../Figures/2Ba_SCAN/TS_SCAN.png?raw=true" alt="SCAN Transition State" width="700"/>
    <figcaption>The transition state as obtained from the SCAN calculation.</figcaption>
</figure>
...
Sometimes a SCAN job may fail. However, rather than resetting the SCAN job from the beginning, you may want your SCAN job to resume from the last converged SCAN image. 
...
5. Open the copied ``orca.inp`` file, and make two changes to this file
    ...
    1. Change the name of the xyz file given in the ``xyzfile`` line to ``last_converged_SCAN_image_restart.xyz``
    ...
    2. In the ``GEOM`` section: 
        ...
        * Change the number of SCAN iterations to perform in order to keep displacement between SCAN images the same. For example, here the original displacement was $\frac{\rm{d_f} - \rm{d_i}}{(\rm{no.\:SCAN\:iterations})-1} = \frac{3.245 - 0.745}{126-1} = 0.02$. The distance between atoms 11 and 16 in ``last_converged_SCAN_image_restart.xyz`` is ``2.085 Å``. Therefore, we need to perform $\frac{d_f(\rm{new}) - d_i}{\rm{displacement}}+1 = \frac{2.085 - 0.745}{0.02}+1 = 68$ (The $+1$ is so we include the initial image in the SCAN procedure).

        Examples of the two components of ``GEOM`` that need to be changed are given below
    ...
    * xyzfile 1 1 last_converged_SCAN_image_restart.xyz
    ...
    !!! note
        ...
        I have also included ``MaxIter 200`` in the ``GEOM`` section. This was because I was having convergence issues because the number of geometry optimisations was going over the default value of ``50``. I increased this to ``200`` so that it would run without problems in this example. 
